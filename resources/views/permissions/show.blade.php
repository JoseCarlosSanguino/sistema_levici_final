...
@section('content')
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-9">
                <div class="card">
                    <div class="card-header"><h3> {{ $permission->name }}</h3></div>
                    <br/>
                    <div class="card-body">
                        ...
                        <a href="{{ url('/'.$controller.'/' . $permission->id . '/edit') }}" title="Editar"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Editar</button></a>

                        <form method="POST" action="{{ url('/'.$controller . '/' . $permission->id) }}" accept-charset="UTF-8" style="display:inline">
                            {{ method_field('DELETE') }}
                            {{ csrf_field() }}
