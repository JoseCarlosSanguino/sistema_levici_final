...
<div class="form-group {{ $errors->has('ivacondition_id') ? 'has-error' : ''}}">
    ...
    @foreach ($ivaconditions as $optionKey => $optionValue)
        <option value="{{ $optionKey }}" {{ (isset($provider->ivacondition_id) && $provider->ivacondition_id == $optionKey) ? 'selected' : ''}}>{{ $optionValue }}</option>
    @endforeach
</select>
...
<div class="form-group {{ $errors->has('markup') ? 'has-error' : ''}}">
    ...
    <input class="form-control" type="number" name="markup" type="text" id="markup" value="{{ $provider->markup or '0'}}" >
    {!! $errors->first('markup', '<p class="help-block">:message</p>') !!}
</div>

<div class="form-group {{ $errors->has('observation') ? 'has-error' : ''}}">
    <label for="observation" class="control-label">{{ 'Observaciones' }}</label>
    <input class="form-control" name="observation" type="text" id="observation" value="{{ $provider->observation or ''}}" >
...
<script type="text/javascript">
    ...
    var city_initial_val = {{ $provider->city_id or ''}}
